"use client";
import { motion } from "framer-motion";
import { education, awards } from "../../lib/data";

export const Education = () => {
  return (
    <section className="py-24 px-6 md:px-12 bg-white border-t border-slate-100">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16">
        {/* Education Column */}
        <div>
          <h2 className="text-3xl font-playfair font-bold mb-12 text-slate-900">Education</h2>
          <div className="space-y-12">
            {education.map((edu, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, x: -20 }}
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true }}
              >
                <h3 className="text-xl font-bold text-slate-900">{edu.degree}</h3>
                <p className="text-blue-800 font-medium mt-1">{edu.school}</p>
                <div className="flex justify-between items-center mt-2">
                  <span className="text-slate-400 text-sm">{edu.year}</span>
                </div>
                {edu.note && <p className="text-slate-600 text-sm mt-3 italic">"{edu.note}"</p>}
              </motion.div>
            ))}
          </div>
        </div>

        {/* Awards/Certifications Column */}
        <div>
          <h2 className="text-3xl font-playfair font-bold mb-12 text-slate-900">Honors & Engagement</h2>
          <div className="grid gap-6">
            {awards.map((award, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, x: 20 }}
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="p-6 bg-slate-50 rounded-lg border border-slate-100"
              >
                <h3 className="font-bold text-slate-900">{award.title}</h3>
                <p className="text-slate-600 text-sm mt-1">{award.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
};